...
parser.add_argument('--load_root', default="/media/sail/HDD10T/Vish/Models/segmentation_models", type=str)
...
def get_loss(model, criterion, mri_1, mri_2, fsl_1, fsl_2):
    ...
    brain_map_1 = (fsl_1>0).float()
    brain_map_2 = (fsl_2>0).float()
    ...
    max_projection = max_projection[0].float()

    # Stack this brain map on itself 3 times to make it 3 channels to match output dimensions (after stacking, 3x192x192x192)
    stacked_brain_map = torch.stack([max_projection, max_projection, max_projection], dim=0)
    # stacked_brain_map_1 = torch.stack([max_projection_1[0], max_projection_1[0], max_projection_1[0]], dim=0)

    # Fit model
    output_1 = model(mri_1)
    output_2 = model(mri_2)

    # isolated_image shape: 3x192x192x192
    isolated_image_1 = output_1[0][:][:][:][:]
    isolated_image_2 = output_2[0][:][:][:][:]
    ...
    isolated_image_1 = torch.mul(stacked_brain_map, isolated_image_1)
    isolated_image_2 = torch.mul(stacked_brain_map, isolated_image_2)

    return isolated_image_1, isolated_image_2, stacked_brain_map
...
if __name__ == '__main__':
    ...
    load_dir = args.load_root + '/' + 'total' + '/best_model_TABS_x8.pth'
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(Test_dataloader):
            ...
            mri_1 = mri_1.cuda(args.gpu, non_blocking=True)
            mri_2 = mri_2.cuda(args.gpu, non_blocking=True)
            fsl_1 = fsl_1.cuda(args.gpu, non_blocking=True)
            fsl_2 = fsl_2.cuda(args.gpu, non_blocking=True)

            isolated_image_1, isolated_image_2, stacked_brain_map = get_loss(model, criterion, mri_1, mri_2, fsl_1, fsl_2)

            metrics_maps = tissue_wise_map_metrics(isolated_image_1, isolated_image_2, stacked_brain_map)
            metrics =  tissue_wise_probability_metrics(isolated_image_1, isolated_image_2, stacked_brain_map)
    ...
    log_name = os.path.join(args.root, 'COBRE_TABS.txt')
