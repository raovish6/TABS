...
from models.TABS.Transformer import TransformerModel
from models.TABS.PositionalEncoding import FixedPositionalEncoding,LearnedPositionalEncoding
from models.TABS.Unet_skipconnection import Unet


class TABS_Transformer(nn.Module):
    def __init__(
        self,
        img_dim,
        patch_dim,
        ...
        super(TABS_Transformer, self).__init__()
    ...
    def forward(self, x, auxillary_output_layers=[1, 2, 3, 4]):
        ...
        decoder_output = self.decode(
            x0_1, x1_1, x2_1, x3_1, encoder_output, intmd_encoder_outputs, auxillary_output_layers
        )
...
class TABS_Model(TABS_Transformer):
    def __init__(
        self,
        img_dim,
        patch_dim,
        ...
        super(TABS_Model, self).__init__(
            img_dim=img_dim,
            patch_dim=patch_dim,
        ...
        self.DeUp1 = DeUp_Cat(in_channels=self.embedding_dim//32, out_channels=self.embedding_dim//64)
        self.DeBlock1 = DeBlock(in_channels=self.embedding_dim//64)

        self.endconv = nn.Conv3d(self.embedding_dim // 64, 3, kernel_size=1)


    def decode(self, x0_1, x1_1, x2_1, x3_1, x, intmd_x, intmd_layers=[1, 2, 3, 4]):
        ...
        x8 = self.Enblock8_1(x8) # (1, 128, 12, 12, 12)
        x8 = self.Enblock8_2(x8)

        # print(x8.shape)
        # print(x3_1.shape)
        y4 = self.DeUp4(x8, x3_1)  # (1, 64, 24, 24, 24)
        y4 = self.DeBlock4(y4)
        # print(y4.shape)

        y3 = self.DeUp3(y4, x2_1)  # (1, 32, 48, 48, 48)
        y3 = self.DeBlock3(y3)

        y2 = self.DeUp2(y3, x1_1)  # (1, 16, 96, 96, 96)
        y2 = self.DeBlock2(y2)

        # I added this extra y1 upsampling layer
        y1 = self.DeUp1(y2, x0_1)  # (1, 8, 192, 192, 192)
        y1 = self.DeBlock1(y1)

        y = self.endconv(y1)      # (1, 3, 192, 192, 192)
        y = self.Softmax(y)
...
def TABS(dataset='mri', _conv_repr=True, _pe_type="learned"):
